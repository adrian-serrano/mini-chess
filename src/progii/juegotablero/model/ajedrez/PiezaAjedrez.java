...
public abstract class PiezaAjedrez extends Pieza {
	...
	public boolean movimientoDentroLimites(int toFila, int toColumna) {
		return toFila <= 7 && toColumna <= 7 && toFila >= 0 && toColumna >= 0;
	}
	...
	public boolean puedeMover(int toFila, char toColumna) {
		return this.getFilaAjedrez() != toFila || this.getColumnaAjedrez() != toColumna;
	}
	 ...
	 * 
	 * @return retiorna la columna en la que se encuentra la pieza en un tablero de
	 *         ajedrez
	 */
...
//	métodos auxiliares
	...
	protected void casillasVisitables(ArrayList<Casilla> resultado, int incFila, int incColumna) {
		int contadorFila = this.getFila() + incFila;
		int contadorColumna = this.getColumna() + incColumna;
		boolean hayPiezaEnMedio = false;

		while (movimientoDentroLimites(contadorFila, contadorColumna) && !hayPiezaEnMedio) {

			this.casillaVisitable(resultado, contadorFila, contadorColumna);

			if (PiezaAjedrez.queHay(contadorFila, contadorColumna) != null) {
				hayPiezaEnMedio = true;
			}
			contadorFila += incFila;
			contadorColumna += incColumna;
		}
	}
