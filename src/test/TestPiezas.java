...
import progii.juegotablero.model.ajedrez.FactoriaPiezasAjedrez;
...
public class TestPiezas extends PartidaAjedrez {
	...
	public TestRule globalTimeout = new DisableOnDebug(Timeout.millis(200)); // 100
																				// milisegundos
																				// máximos
																				// por
																				// test
	...
	//2018-12-03 ALGM: Test para verificar que se detecta como movimiento
	//no válido el moverse a la misma casilla
	/**
	 * Este test verifica que no se dé como valida la misma casilla de la que se parte
	 */
	@Test
	public void testProbarMismaCasilla() {
		String []piezas = {"ALFIL", "CABALLO", "PEON",
				"REINA","REY","TORRE"};
		try {
			for (String tipoPieza : piezas) {
				PiezaAjedrez pieza = (PiezaAjedrez) FactoriaPiezasAjedrez.crearPieza(new ControlJugadoresAjedrez(),
						6, 'a', tipoPieza, "BLANCO");
				assertFalse("No es movimiento valido ir a la misma casilla de partida",
						pieza.puedeMover(pieza.getFilaAjedrez(), pieza.getColumnaAjedrez()));	
				pieza = (PiezaAjedrez) FactoriaPiezasAjedrez.crearPieza(new ControlJugadoresAjedrez(),
						3, 'h', tipoPieza, "NEGRO");
				assertFalse("No es movimiento válido ir a la misma casilla de partida",
						pieza.puedeMover(pieza.getFilaAjedrez(), pieza.getColumnaAjedrez()));
			}
	...
	protected PiezaAjedrez nuncaHayPieza(Integer fila, Character columna) {
		return null;
	}
